...
public:
  DeclarationResolver(
      std::shared_ptr<TypeTable> types, std::shared_ptr<Scope> top_scope,
  ...
  virtual void resolve() override {
    ...
    for (auto header : headers) {
      ...
      if (!header) {
        std::cerr << "header is nullptr!" << std::endl;
      }
      try {
        header->accept(this);
  ...
  visitTypeDef(CflatParser::TypeDefContext *ctx) override {
    ...
    std::shared_ptr<Type> user_type =
        std::make_shared<UserType>(type_name, real_type);
  ...
  visitTypeRef(CflatParser::TypeRefContext *ctx) override {
    auto base_type =
        ctx->typeRefBase()
            ->accept(this)
            .as<std::shared_ptr<
                Type>>(); // 一定是有的，且一定是從typetable中取出來的
    assert(base_type);

    for (auto suffix : ctx->typeRefSuffix()) {
      std::shared_ptr<Type> type =
          suffix->accept(this).as<std::shared_ptr<Type>>();

      // 先看看是否是函數
      std::shared_ptr<FunctionType> funType = std::shared_ptr<FunctionType>(
          dynamic_cast<FunctionType *>(type.get()));
      if (funType) {
      ...
      std::shared_ptr<ArrayType> arrayType =
          std::shared_ptr<ArrayType>(dynamic_cast<ArrayType *>(type.get()));
      if (arrayType) {
      ...
      std::shared_ptr<PointerType> pointerType =
          std::shared_ptr<PointerType>(dynamic_cast<PointerType *>(type.get()));
      if (pointerType) {
  ...
  visitArrayTypeSuffix(CflatParser::ArrayTypeSuffixContext *ctx) override {
    ...
    std::shared_ptr<Type> array = std::make_shared<ArrayType>(
        std::shared_ptr<Type>(nullptr), n, 8); // 暫時未知base_type
  ...
  visitPointerTypeSuffix(CflatParser::PointerTypeSuffixContext *ctx) override {
    std::shared_ptr<Type> pointer =
        std::make_shared<PointerType>(8, std::shared_ptr<Type>(nullptr));
  ...
  virtual antlrcpp::Any visitFunctionTypeSuffix(
    ...
    std::shared_ptr<Type> fun = std::make_shared<FunctionType>(
        std::shared_ptr<Type>(nullptr), p); // todo
