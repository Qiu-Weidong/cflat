...
public:
  UserType(const std::string &name, std::shared_ptr<Type> real_type)
      : real_type(real_type) {
    this->name = name;
  }

  std::shared_ptr<Type> getRealType() const { return real_type; }
  void setRealType(std::shared_ptr<Type> real_type) {
  ...
  virtual bool isAllocatedArray() const override {
    return real_type->isAllocatedArray();
  }
  virtual bool isIncompleteArray() const override {
    return real_type->isIncompleteArray();
  }
  virtual bool isScalar() const override {
    return real_type->isScalar();
  } // scalar 在 C 语言中是整数类型、指针以及枚举类型的总称
  ...
  virtual bool operator==(const Type &other) {
    return real_type->operator==(other);
  }

  friend std::ostream &operator<<(std::ostream &os, const UserType &type) {
    os << "{ User Type " << type.name << " -> " << *(type.real_type) << "}";
    return os;
  }
};
