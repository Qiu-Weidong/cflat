...
public:
  CompositeType(const std::string &name,
                const std::map<std::string, std::shared_ptr<Type>> &members)
      : members(members) {
    this->name = name;
  }
  ...
  std::map<std::string, std::shared_ptr<Type>> getMembers() const {
    return members;
  }
  ...
  void setMembers(const std::map<std::string, std::shared_ptr<Type>> &members) {
    this->members = members;
  }
  bool insertMember(const std::string &name, std::shared_ptr<Type> type) {
    return members.insert(std::make_pair(name, type)).second;
  }
  bool insertOrAssignMember(const std::string &name,
                            std::shared_ptr<Type> type) {
    return members.insert_or_assign(name, type).second;
  }

  friend std::ostream &operator<<(std::ostream &os,
                                  const CompositeType &composite) {
    os << "{ Composite Type " << composite.name << " -> members: ";
    for (auto it = composite.members.begin(); it != composite.members.end();
         it++) {
