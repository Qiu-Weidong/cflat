...
public:
  ...
  bool defineType(const std::string &name, std::shared_ptr<Type> type) {
    return table.insert(std::make_pair(name, type)).second;
  }
  bool defineTypeForce(const std::string &name, std::shared_ptr<Type> type) {
    return table.insert_or_assign(name, type).second;
  }
